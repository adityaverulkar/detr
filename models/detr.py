...
class DETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False):
        ...
        Parameters:
            ...
            num_queries: number of object queries, i.e., detection slots. This is the maximal number of objects
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # We prepare the targets and outputs for each level
        indices = self.matcher(outputs_without_aux, targets)
        num_boxes = sum(len(t["labels"]) for t in targets)
        num_boxes = torch.as_tensor(num_boxes, dtype=torch.float, device=outputs['pred_logits'].device)
        num_boxes = torch.max(num_boxes, torch.ones_like(num_boxes))  # ensure non-zero denominator
...
class MLP(nn.Module):
    """MLP class. This module is used to predict bounding box coordinates."""
